...
working_directory "/usr/local/git/gitlab" # available in 0.94.0+

# Listen on both a Unix domain socket and a TCP port.
# If you are load-balancing multiple Unicorn masters, lower the backlog
# setting to e.g. 64 for faster failover.
listen "/usr/local/git/gitlab/tmp/sockets/gitlab.socket", :backlog => 1024
...
pid "/usr/local/git/gitlab/tmp/pids/unicorn.pid"

# By default, the Unicorn logger will write to stderr.
# Additionally, some applications/frameworks log to stderr or stdout,
# so prevent them from going to /dev/null when daemonized here:
stderr_path "/usr/local/git/gitlab/log/unicorn.stderr.log"
stdout_path "/usr/local/git/gitlab/log/unicorn.stdout.log"
...
require_relative "/usr/local/git/gitlab/lib/gitlab/log_timestamp_formatter.rb"
